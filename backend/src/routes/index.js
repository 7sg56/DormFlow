...
module.exports = async function routes(fastify) {
  ...
  // ── All routes below require auth (encapsulated scope) ─────
  await fastify.register(async function protectedRoutes(app) {
    app.addHook('preHandler', requireAuth);

    // ── Dashboard (role-specific stats & actions) ──────────────
    await app.register(require('./dashboard.routes'), { prefix: '/dashboard' });

    // ── Allocations (custom routes) ────────────────────────────
    await app.register(require('./allocation.routes'), { prefix: '/allocations' });

    // ── Generic CRUD Entities ──────────────────────────────────

    await app.register(createCrudRoutes({
